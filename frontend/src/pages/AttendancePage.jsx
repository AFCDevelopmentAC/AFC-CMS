...
export default function AttendancePage() {
  ...
  async function load() {
    ...
    try {
      const { data } = await api.get(
        `/api/attendance/roster/${sessionType}/${sessionId}`
      );
      setSession(data);
      const r = data.roster || [];
      setRoster(r);
      const presentSet = new Set(
        r.filter(m => m.is_present).map(m => m.member_sn)
      );
      setMarked(new Set(presentSet));
      setOriginal(new Set(presentSet));
    } catch (err) {
      setError(err?.response?.data?.detail || "Could not load attendance roster.");
  ...
  async function saveAttendance() {
    ...
    try {
      const toMark = roster
        .filter(m => marked.has(m.member_sn) && !original.has(m.member_sn))
        .map(m => m.member_sn);

      const toUnmark = roster
        .filter(m => !marked.has(m.member_sn) && original.has(m.member_sn))
        .map(m => m.member_sn);
  ...
  return (
    <div className="att-page">
      ...
      {saved && <div className="att-saved">Attendance saved successfully.</div>}
